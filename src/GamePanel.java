...
public class GamePanel extends JPanel implements ActionListener, KeyListener {
	...
	GamePanel() {
		...
		one = new CovidBlob (getX() + 550, getY(), 50, 50);
		frameDraw.start();
	}
...
tier = 0;
	...
	void drawEndState(Graphics g) {
		if (success == false) {
			...
			g.setColor(Color.BLACK);
			g.drawString("You have dodged " + timesDodged + " Coronavirus bacteria samples.", 100, 250);
			g.drawString("Infection date: Tier " + tier + ".", 100, 300);
			if(tier == 0) {
				g.drawString("Wimp!", 100, 350);
			}
			if(tier == 1) {
				g.drawString("Ok, not bad.", 100, 350);
			}
			g.setFont(smallerText);
			g.setColor(Color.BLACK);
		...
		if (e.getKeyCode() == KeyEvent.VK_SPACE) {
			if (currentState == MENU) {
				...
				JOptionPane.showMessageDialog(null,
						"Use the arrow keys to dodge the COVID that come from the right side of the screen.");
				JOptionPane.showMessageDialog(null,
						"You have to make it through the yellow tier, the orange tier, and the purple tier to win.");
				JOptionPane.showMessageDialog(null,
						"If you get hit three times, you are 100% infected, and the game is over.");
